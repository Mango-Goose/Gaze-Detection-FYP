...
#command line argument for path to the Gazefollow dataset but uses ./gazefollow_extended as default
...
def main(DATA_PATH):
    ...
    #reads the training annotations file

    #!!!remove nrows from this line when training on server!!!
    
    df = pd.read_csv(train_csv_path, header=None, names=column_names, index_col=False, nrows=100)
    df = df[df['inout'] != -1]  #filter out entries with inout == -1
    ...
    for path, row in df.iterrows():
        ...
        #loops through each person in image
        #pulls out raw bounding box and gaze coords for each
        #fixes any inverted bounding boxes
        #clamps bounding box coordinates to be within image bounds
        for i in range(num_people):
            ...
            #move in and out of frame bbox annotations
            xmin = max(xmin, 0)
            ymin = max(ymin, 0)
            xmax = min(xmax, width)
            ymax = min(ymax, height)

            #stores the relative image path, list of heads, number of heads
            heads.append({
     ...
     #summary of train set and writes it all to the new JSON file
